...
- `-t` or `--total`: Maximum number of results to scrape (default: 1). Scrolling stops early when the result list runs out, so a high value means "scrape everything available".
...
- `--emails`: Visit each business website and extract an email address (default: off). Google Maps does not publish emails, so this is the only way to get them. Adds a few seconds per business.
- `--require-contact`: Drop results that have neither a phone number nor an email (default: off)
...
Scrape everything available, keeping only businesses you can actually contact:
```bash
python main.py -s "Turkish Restaurants in Toronto Canada" -t 300 -o leads.csv --emails --require-contact
```
